...
- [1. 冷启动对象](#sec-1)
  - [1.1 Surface-specific Cold Start](#sec-1-1)
...
### 1.1 Surface-specific Cold Start
...
ID embedding 的常见 fallback 有三种：

1. Shared default embedding：所有未学习 ID 使用统一向量，稳定但无法区分新供给；
2. Neighbor initialization：用内容最相似的成熟 item embedding 加权平均，个性化更强但会继承近邻偏差；
3. Content-to-collaborative mapping：训练编码器从内容/属性直接预测协同空间表示，适合大规模持续上新。
