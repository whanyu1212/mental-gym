...
A sliding window needs a contiguous range whose validity is **monotonic** in the endpoints:

> If `[left, right]` is already invalid, every larger range that still contains it is also invalid. For a shortest-window problem, if `[left, right]` already meets the goal, no larger range can beat it, so the only useful move is to shrink and test whether a tighter range still qualifies.

Do **not** force a window when:

| Signal | Why the window fails | Reach for |
|---|---|---|
| The values can be negative or zero and the test is a sum / product | Adding a value can help *or* hurt, so shrinking is no longer a safe repair | [Prefix Sum](../prefix_sum_pattern/) (count ranges that hit a target) or [Kadane](../kadane_algorithm/) (best contiguous sum) |
...
A single `if` can replace the `while` here, and only here. The window grows by one character per step and the task asks for a *longest* valid range, so shrinking by more than one cannot beat a width you have already recorded. A stale `max_f` is also safe for the same reason: it can only make the validity test stricter. Use `while` until that argument is obvious; the saved 424 solution uses the `if` form.
