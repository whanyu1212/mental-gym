...
A sliding window needs a contiguous range whose validity is **monotonic** in the endpoints, but the direction depends on the prompt:
...
Do **not** force a window when:

| Signal | Why the window fails | Reach for |
|---|---|---|
| A sum-threshold input can contain negative values | Expanding can decrease the sum and shrinking can increase it, so endpoint movement is no longer monotonic. Zero is safe for the usual non-negative sum windows. | [Prefix Sum](../prefix_sum_pattern/) (count ranges that hit a target) or [Kadane](../kadane_algorithm/) (best contiguous sum) |
| A product input violates the positivity assumptions used by the window proof | Zero, negative values, or factors below one can change how multiplication responds to expansion and shrinking; derive the product-specific invariant before using a window. | A product-specific prefix/transform approach, or another method justified by the input constraints |
