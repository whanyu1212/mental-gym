...
The seed is written as a generator expression on purpose. `sum(arr[:k])` reads better but allocates a fresh `k`-element list first, which costs O(k) auxiliary space and quietly breaks the O(1) claim whenever `k` grows with the input. Iterating indices keeps the seed O(1).
...
| Handles `k > len(arr)` | yes, no window ever reaches width `k` | yes, `right` never reaches `k - 1` | **no** — the seed silently totals fewer than `k` elements |
