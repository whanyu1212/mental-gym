...
|---|---|---|
| A sum-threshold input can contain negative values | Expanding can decrease the sum and shrinking can increase it, so a threshold cannot safely decide which boundary to move. Zero is safe for the usual non-negative sum windows. | [Prefix Sum](../prefix_sum_pattern/) (count ranges that hit a target) or [Kadane](../kadane_algorithm/) (best contiguous sum) |
