...
A single `if` can replace the `while` here, and only here. The window grows by one character per step and the task asks for a *longest* valid range, so shrinking by more than one cannot beat a width you have already recorded.
