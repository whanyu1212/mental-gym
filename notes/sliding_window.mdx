...
import ContainsDuplicate2Animation from '../web/src/components/ContainsDuplicate2Animation.astro';
...
| Nearby duplicate, or any predicate over the last `k` values | Fixed-size membership window | A set or map of the values still in range | [Contains Duplicate II](../../algorithms/219-contains-duplicate-ii) |
| Check every substring of exactly length `k` | Fixed-size summary window | Counts, sum, or another rolling summary | [Permutation in String](../../algorithms/567-permutation-in-string) |
| Longest valid substring/subarray | Variable-size window | State that says whether the current range is valid | [Longest Substring Without Repeating Characters](../../algorithms/3-longest-substring-without-repeating-characters) |
| Shortest range that still satisfies a goal | Shrink-to-qualify window | The same validity state, but the answer is taken while shrinking | See [Shortest Windows Update Inside The Repair Loop](#shortest-windows-update-inside-the-repair-loop) |
...
| 2 | [Contains Duplicate II](../../algorithms/219-contains-duplicate-ii) | A set that is exactly the last `k` values |
| 3 | [Number of Sub-arrays of Size K and Average >= Threshold](../../algorithms/1343-number-of-sub-arrays-of-size-k-and-average-greater-than-or-equal-to-threshold) | A rolling sum of fixed width |
| 4 | [Permutation in String](../../algorithms/567-permutation-in-string) | Slide a fixed-width frequency map |
| 5 | [Longest Substring Without Repeating Characters](../../algorithms/3-longest-substring-without-repeating-characters) | Grow, then shrink until the invariant is restored |
| 6 | [Longest Repeating Character Replacement](../../algorithms/424-longest-repeating-character-replacement) | Express validity as a repair budget |
| 7 | [Sliding Window Maximum](../../algorithms/239-sliding-window-maximum) | Keep only candidates that can still win |

[Permutation in String](../../algorithms/567-permutation-in-string) and [Sliding Window Maximum](../../algorithms/239-sliding-window-maximum) are currently filed in the Stack section of the practice bank, but their core technique belongs in this sliding-window progression.
...
### Shortest Windows Update Inside The Repair Loop

The skeleton above is the **longest**-window rhythm: grow, repair until the range is valid, then take the current width as a candidate. Shortest-window prompts invert only the moment you record the answer.
...
for right, value in enumerate(items):
    ...
    while window_meets_goal(state, left, right):
        best = min(best, right - left + 1)
        remove(state, items[left])
        left += 1
```

The window is useful *while it still qualifies*. Each shrink asks whether a tighter range can still meet the goal. Recording after the `while` would measure a window that has already been trimmed *past* the last valid left edge.

Minimum Size Subarray Sum is the usual first example: all values are positive, so adding on the right can only help the sum and removing on the left can only hurt it. That monotonicity is what makes the inner `while` safe.

### When Not To Use This

A **variable-size** sliding window whose boundary movement depends on validity needs a contiguous range whose validity is **monotonic** in the endpoints, but the direction depends on the prompt. Fixed-size windows are different: the required width determines each eviction, so their values may be negative without breaking the add/evict technique.

- **Longest-window constraints are downward-closed.** If a window is valid, every subwindow is valid; equivalently, once a window is invalid, every larger range containing it is also invalid. Shrinking repairs the constraint.
- **Shortest-window goals are upward-closed.** If a window meets the goal, every larger range containing it also meets the goal; a smaller window may stop qualifying. Expanding can turn a miss into a match, and shrinking searches for the tightest match.

Do **not** force a variable grow/shrink window when:

| Signal | Why the window fails | Reach for |
|---|---|---|
| A sum-threshold input can contain negative values | Expanding can decrease the sum and shrinking can increase it, so a threshold cannot safely decide which boundary to move. Zero is safe for the usual non-negative sum windows. | Prefix sums plus a monotonic deque (shortest range reaching a threshold), a [prefix-sum frequency map](../prefix_sum_pattern/) (count/exact-target ranges), or [Kadane](../kadane_algorithm/) (best contiguous sum) |
| A product input violates the positivity assumptions used by the variable-window proof | Zero, negative values, or factors below one can change how multiplication responds to expansion and shrinking; derive the product-specific invariant before using a window. | A product-specific prefix/transform approach, or another method justified by the input constraints |
| The prompt asks for a subsequence, not a substring / subarray | The answer is allowed to skip elements | Hashing, DP, or two pointers on a transformed input |
| You must count *every* valid range, not the best one, and validity is not a simple prefix of the current window | A best-so-far scalar throws the extras away | Either a window that counts `right - left + 1` starts after each repair, or a prefix-sum frequency map |
| The useful left partner is not “the current range,” only “the best earlier value” | There is no range invariant to restore | The forward-pair scan in [section 1](#1-forward-pair-scan-best-time-to-buy-and-sell-stock), or Kadane on daily differences |

If you cannot name the property that makes the next `left += 1` safe, stop and write the invariant before coding.
...
- "nearby / within `k` positions"
- "contains a permutation/anagram"
- "every substring of length `k`"
- "average, sum, or count in every block of `k` values"

The window does not choose its own width. It advances exactly one step at a time after it reaches the required size. The state is what changes from problem to problem: a membership set, a rolling sum, or a frequency map.

### First Anchor: Contains Duplicate II

[Contains Duplicate II](../../algorithms/219-contains-duplicate-ii) asks whether any value repeats inside an index distance of `k`. The window is the last `k + 1` values. Membership in that window *is* the distance constraint, so a hit is a valid nearby pair.

<PatternVizModal label="Fixed-Size Membership Window">
  <ContainsDuplicate2Animation />
</PatternVizModal>
...
Just before testing index `right`:

```text
window holds exactly the values at indices max(0, right - k) .. right - 1.
If value is already in the set, a pair at distance <= k exists.
```

Remove `nums[left]` **before** incrementing `left`. Advance first and you evict the wrong element; the truly stale value stays in the set and later reports a duplicate that is farther than `k`.

`k = 0` is always false: distinct indices cannot satisfy `abs(i - j) <= 0`.

### Change One Rule

If the question asks for a rolling **sum** of every block of `k` values instead of membership, keep the same skeleton and replace the set with a running total. That is [Number of Sub-arrays of Size K and Average >= Threshold](../../algorithms/1343-number-of-sub-arrays-of-size-k-and-average-greater-than-or-equal-to-threshold), worked in [Three Ways To Write The Same Fixed Window](#three-ways-to-write-the-same-fixed-window) below.

### Next Anchor: Permutation In String
...
Comparing whole maps is fine while the alphabet is a small constant. For a true O(1) check per slide, keep a `matches` counter: how many distinct characters currently have the exact needed frequency. Adding or removing a character then updates `matches` in constant time, and the window is an anagram exactly when `matches == len(needed)`.

### Three Ways To Write The Same Fixed Window

A fixed-size window is the one case where `left` is *predictable*: it is always exactly `max(0, right - k + 1)`. The clamp matters — during the first `k - 1` steps the window is still filling up and `left` stays at `0`, which is where the warm-up guards in the code below come from. Once the window is full, the `max` never binds again and the left edge is simply `right - k + 1`. That closed form means the left edge can be written down explicitly, computed on the fly, or dropped entirely. All three forms below are the same algorithm — same O(n) time, same O(1) extra state, same number of additions and subtractions. They differ only in how much of the window's position is stored versus re-derived.
...
```python
if len(arr) < k:                             # mandatory here — see below
    return 0

window_sum = sum(arr[i] for i in range(k))   # not sum(arr[:k]) — see below
...
Two details in that seed are deliberate. The generator expression avoids `sum(arr[:k])`, which reads better but allocates a fresh `k`-element list first — O(k) auxiliary space, which would quietly break the O(1) claim whenever `k` grows with the input. And the length guard is not optional: seeding assumes `k` elements exist, so without it the generator walks off the end and raises `IndexError`. Forms A and B need no such guard because their loops simply never reach a full window.

### Which To Reach For

| | Form A: explicit | Form B: implicit | Form C: left-driven |
|---|---|---|---|
| Branches in the loop | 2 — evict + qualify | 2 — evict + qualify | 1 — qualify only |
| Setup before the loop | none | none | seed the first window |
| Handles `k > len(arr)` | yes, for free — no window ever reaches width `k` | yes, for free — `right` never reaches `k - 1` | **only with the explicit guard**; without it the seed raises `IndexError` |
| Generalizes to variable windows | yes | no | no |

Every form pays for the qualification check; what Form C avoids is the *warm-up* branch, because seeding the first window means it never runs on a partial one.

Form B's two guards, `right >= k` and `right >= k - 1`, are Form A's two conditions with `left` substituted away: `right - left + 1 > k` becomes the first, `right - left + 1 == k` becomes the second. Neither guard disappears in the translation — they map one-to-one at every step.

The substitution needs care, because `left` changes value *within* an iteration and each guard reads it at a different moment:

| Guard in Form A | When it reads `left` | `left` equals | Reduces to |
|---|---|---|---|
| `right - left + 1 > k` | before the eviction | `max(0, right - k)` | `right >= k` |
| `right - left + 1 == k` | after the eviction | `max(0, right - k + 1)` | `right >= k - 1` |

Using the post-eviction formula for both is the tempting mistake: it caps the width at `k`, so `> k` could never fire and the eviction branch would look like dead code. The eviction guard is asking about the window *before* it has been trimmed. Once each guard is paired with the right formula the substitution is mechanical, which is why the off-by-one between `>= k` and `>= k - 1` looks arbitrary until you write `left` back in and see the two different moments it came from.
...
### Change One Rule: Jump Instead Of Stepping

The `while` is honest, but it is not required. Store `last_seen[char] = index` and, when a repeat arrives, jump:

```python
left = max(left, last_seen[char] + 1)
```

The `max` is the `"abba"` lesson in one expression: a later last-seen index must not pull `left` backward over a shrink you have already paid for.
...
A single `if` can replace the `while` only when you have a problem-specific **non-shrinking best-width proof**; the optimization is not unique to this problem. Some longest, downward-closed windows support it when each step adds one right value, can evict one left value, and any temporarily invalid maintained range cannot increase the recorded answer beyond an attainable valid width. It is unsafe when later logic needs the maintained range or left boundary to be exactly valid—such as many shortest, counting, or enumeration problems—or whenever no proof rules out an invalid range inflating the answer. Default to `while` until those conditions are proved.

A non-decreasing historical `max_f` needs a different proof. Once stale, it is at least the current window's true maximum, so it **underestimates** the replacement cost and makes the validity test more permissive. The shortcut is still safe for the maximum length: the algorithm never shrinks a previously achieved candidate width merely to restore exact validity, and that width can grow only when a newly observed frequency provides enough real matching characters to justify the larger candidate. The maintained range may therefore be temporarily invalid, even though the recorded best width remains attainable. Use the recomputed maximum and `while` form until that non-shrinking best-width argument is obvious; the saved 424 solution uses the optimized `if` form.
...
If the task asks about a **contiguous range** and a condition that can be updated as endpoints move, investigate sliding window first. If the range is contiguous but the next shrink is not a safe repair, start with [Prefix Sum](../prefix_sum_pattern/) or [Kadane](../kadane_algorithm/) instead. For the Python helper that usually implements a window maximum, see [Python DSA Toolkit](../python-dsa-toolkit/).
...
| A nearby-duplicate check fires too late or too early | Is the eviction test `right - left > k`, and did I evict `nums[left]` before incrementing? |
...
| A shortest-window answer is one step too long | Did I record the width *inside* the shrink loop, while the window still qualified? |
...
| Fixed-size membership window | O(n) | O(min(n, k)) | Each value enters and leaves the set at most once |
| Fixed-size sum window | O(n) | O(1) | One addition and one subtraction per slide |
...
| Shortest qualifying window | O(n) | O(1) or O(alphabet) | `left` still only moves forward |
...
4. When is it safe to update the answer — after the repair, or while shrinking?
5. Does each item enter and leave the state at most once?
6. Do I need all window values, or only undominated candidates?
7. Is the validity rule actually monotonic, or should this be prefix sums / Kadane?
