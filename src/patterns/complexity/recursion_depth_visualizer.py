...
This module demonstrates and visualizes recursion depth limits and stack
overflow scenarios with interactive examples.
"""
...
def compare_recursion_styles(n: int = 10) -> None:
    ...
    print("  Pending operations: n * (n-1) * ... builds up")
    ...
    print("  Note: Python doesn't optimize tail calls!")

    # Iterative
    result_iter = factorial_iterative(n)
    print("\nIterative:")
    print(f"  Result: {result_iter}")
    print("  Max depth: 0 (no recursion)")
...
def demonstrate_practical_limits() -> None:
    """Show practical scenarios where recursion depth becomes an
    issue."""
    print("\n" + "=" * 60)
    ...
    try:
        ...
        print("  Recursive sum: RecursionError!")
        print("  Status: ✗ Exceeds recursion limit")

    # Iterative solution
    result_iter = deep_list_sum_iterative(large_list)
    print(f"  Iterative sum: {result_iter}")
    print("  Status: ✓ No depth limit")
